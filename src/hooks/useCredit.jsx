    ...
    await axios
      ...
      .then((response) => {
        ...
        if (response.data.response.status) {
          Cookies.set("paymentId", response.data.response.data.paymentid, {
            expires: 1,
          });
  ...
  const applyPromo = async () => {
    ...
    let promoPercentage =
      response.data.response.data.length > 0
        ? response.data.response.data[0].Percentage
        : 0;
    ...
    if (!promoApplied && promoPercentage !== 0) {
      setCredit({
        total: (
          credit.amount -
          (credit.amount / 100) * promoPercentage
        ).toFixed(2),
      });
      setPromoApplied(true);
      setPromoPercentage(promoPercentage);
      setErrors({});
