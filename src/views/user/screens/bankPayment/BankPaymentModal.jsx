...
const BankPaymentModal = ({
  ...
  const updatePayment = async (ContractId, paymentId) => {
    ...
    await axios
      .post(baseURL + "/api/updatepaymentwithcontractid", {
  ...
  const createTicketHandler = async (PaymentId) => {
    await axios
      .post(
        baseURL + "/api/addticket",
        {
          Subject: `Payment request by ${user.ClientName}`,
          Description: `Payment Request by ${user.ClientName} % Payment ID: ${
  ...
  const existingContract = async () => {
    ...
    if (response.data.response.data.length > 0) {
      setCurrentContract(
        response.data.response.data.filter((record) => record.Status == 1)[0]
      );
  ...
  useEffect(() => {
    setAmount(price - (price * promoPercentage) / 100);
  }, [promoPercentage]);
  ...
  return (
    <Modal toggleModal={toggleModal}>
      <div className="user_createTicketForm">
        <div className="user_createTicketForm">
          <H3
            text={t("UserPanel.Billing.BankTransfer")}
                ...
                <tr>
                  ...
                  <td>
                    <FaRegClipboard
                      ...
                      cursor={"pointer"}
                      onClick={() =>
                        copyDetails(creditCardDetails.accountNumber)
                      }
                    />
                  </td>
                </tr>
                ...
                <tr>
                  ...
                  <td>
                    <FaRegClipboard
                      fontSize={20}
                      cursor={"pointer"}
                      onClick={() => copyDetails("7032476751")}
                    />
                  </td>
                </tr>
                <tr>
          ...
          <div className="">
            <h6 className="">
              <b>{selectedPackage && selectedPackage.PackageName}</b>
              <br /> Package for {selectedPackage && selectedPackage.UserType}
            </h6>
